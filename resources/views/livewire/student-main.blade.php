<main class="col-span-5 pl-8 pr-7 py-4 flex flex-col h-dvh gap-4 overflow-y-none">
    ...
    <div class="flex mt-4 gap-2 w-auto justify-between">
        ...
        <div class="flex gap-4 self-start">
            <button wire:click="openClassRecordModal"
                class="relative flex items-center justify-center bg-white py-3 px-3 rounded-full gap-2 text-paragraph cursor-pointer border-2 border-white hover:border-blue-button hover:text-white hover:bg-blue-button"
                wire:loading.attr="disabled" wire:target="openClassRecordModal">

                <div class="flex items-center gap-2" wire:loading.class="invisible" wire:target="openClassRecordModal">
                    <span class="material-symbols-rounded">export_notes</span>
                </div>

                <div wire:loading wire:target="openClassRecordModal" role="status"
            ...
            <button wire:click="openAdvanceStudentModal"
                class="relative flex items-center justify-center bg-white py-3 px-3 rounded-full gap-2 text-paragraph cursor-pointer border-2 border-white hover:border-blue-button hover:text-white hover:bg-blue-button"
                wire:loading.attr="disabled" wire:target="openAdvanceStudentModal">

                <div class="flex items-center gap-2" wire:loading.class="invisible"
                    wire:target="openAdvanceStudentModal">
                    <span class="material-symbols-rounded">upgrade</span>
                    {{-- <p class="">Advance</p> --}}
                </div>

                <div wire:loading wire:target="openAdvanceStudentModal" role="status"
                    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
                    <svg aria-hidden="true" class="w-6 h-6 text-gray-200 animate-spin fill-blue-600"
                        viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
            ...
            <button wire:click="openAddOldStudentModal"
                ...
                wire:loading.attr="disabled" wire:target="openAddOldStudentModal">

                <div class="flex items-center gap-2" wire:loading.class="invisible"
                    wire:target="openAddOldStudentModal">
                    <span class="material-symbols-rounded">add</span>
                    <p class="">Old Student</p>
                </div>

                <div wire:loading wire:target="openAddOldStudentModal" role="status"
    ...
    <div class="mt-12 flex flex-col gap-4 min-h-[20%]">
        <div class="side flex items-center justify-between gap-2 mb-2">
            ...
            <livewire:class-record-modal />
            <livewire:student-advance-modal />
            ...
            <div class="flex items-center gap-4 self-start">
                ...
                <div
                    ...
                    <select name="" id="" class="w-max outline-none"
                        ...
                        <option value="unqualified" class=" text-paragraph">
                            Unqualified
                        </option>
                        ...
                        <tbody>
                            @forelse ($students as $student)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-center text-paragraph flex justify-center items-center">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center items-center gap-1 text-white">
                                            <button wire:click='openEditStudentModal({{ $student->id }})'
                                                ...
                                                {{ !$student->fourthQuarterHasEnded($school_year) ? 'bg-danger hover:scale-110 cursor-pointer' : 'bg-gray-400 cursor-not-allowed' }}"
                                                {{ $student->fourthQuarterHasEnded($school_year) ? 'disabled' : '' }}>
