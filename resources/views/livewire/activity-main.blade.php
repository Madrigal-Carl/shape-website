<main class="col-span-7 pl-8 pr-10 py-4 flex flex-col h-dvh gap-16 overflow-y-none">
    ...
    <div class="flex mt-4 gap-2 w-auto justify-between">
        ...
        <!-- Buttons -->
        <button wire:click="openAddActivityModal"
            ...
            wire:loading.attr="disabled" wire:target="openAddActivityModal">

            <!-- Normal state -->
            <div class="flex items-center gap-2" wire:loading.class="invisible" wire:target="openAddActivityModal">
                <span class="material-symbols-rounded">add</span>
                <p class="">Add Activities</p>
            </div>

            <!-- Loading spinner -->
            <div wire:loading wire:target="openAddActivityModal" role="status"
    ...
    <div class="flex flex-col gap-4 min-h-[20%]">
        <div class="side flex items-center justify-between gap-2 mb-2">
            <h1 class="text-4xl font-bold">Activity List</h1>
        ...
        <div class="flex flex-col min-h-[20%] p-6 bg-white rounded-3xl">
            <div class="flex flex-col overflow-y-scroll">
                <div class="flex flex-col bg-whitel rounded-3xl bg-white">
                    <table class="table-auto border-separate relative">
                        ...
                        <tbody>
                            @forelse ($activities as $activity)
                                <tr>
                                    <td class="px-4 py-3 text-center text-paragraph">{{ $activity->id }}</td>
                                    <td class="px-4 py-3 text-center text-paragraph">{{ ucwords($activity->name) }}</td>
                                    ...
                                    <td class="px-4 py-3 text-center text-paragraph">
                                        <div class="flex justify-center items-center gap-1 text-white">
                                            <button wire:click='openEditActivityModal({{ $activity->id }})'
                                                ...
                                                {{ $this->isEditable($activity) ? 'bg-danger cursor-pointer hover:scale-110' : 'bg-gray-400 cursor-not-allowed' }}"
                                                {{ $this->isEditable($activity) ? '' : 'disabled' }}>

                                                <!-- Text (hidden when loading) -->
                                                <small class="transition-opacity duration-150"
                                                    wire:loading.class="opacity-0"
                                                    wire:target='openEditActivityModal({{ $activity->id }})'>
                                                    Edit
                                                </small>
                                            ...
                                            <button wire:click='openViewActivityModal({{ $activity->id }})'
                                                ...
                                                <small class="transition-opacity duration-150"
                                                    wire:loading.class="opacity-0"
                                                    wire:target='openViewActivityModal({{ $activity->id }})'>
                                                    View
                                                </small>
                    ...
                    <livewire:activity-add-modal />
                    <livewire:activity-edit-modal />
                    <livewire:activity-view-modal />
                </div>
            </div>
        </div>
        ...
        @if ($activities->lastPage() > 1)
            <div class="rounded-full bg-white gap-1 p-2 w-fit self-center-safe flex items-center text-sm shadow-2xl">
                <button
                    class="cursor-pointer py-1 flex items-center px-3 {{ $activities->onFirstPage() ? 'hidden' : '' }}"
                    @if (!$activities->onFirstPage()) wire:click="gotoPage({{ $activities->currentPage() - 1 }})" @endif>
                ...
                <button
                    class="cursor-pointer py-1 flex items-center px-3 {{ $activities->hasMorePages() ? '' : 'hidden' }}"
                    @if ($activities->hasMorePages()) wire:click="gotoPage({{ $activities->currentPage() + 1 }})" @endif>
                    <span class="material-symbols-outlined">
                        chevron_right
                    </span>
                </button>
